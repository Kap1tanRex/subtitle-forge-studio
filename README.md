...
ширину, справа одна колонка со всем остальным. В колонке пять вкладок:
«Список» (таблица реплик с поиском, под ней текст выделенной реплики),
«Реплика» (время с шагом по кадру, пауза до следующей, свойства, пометка и
заметка), «Оформление», «Проверки» (со счётчиком замечаний на вкладке) и
«Актёры». Любую можно вынести в отдельное окно кнопкой в углу колонки.
...
файл. Правка уходит в субтитры сразу, без отдельной кнопки: покрутили кегль —
кадр и таблица уже с новым.

Куда именно ложится оформление, решает выделение. Ничего не выделено — правится
сам стиль, а с ним все реплики на нём. Выделены реплики — оформляются только
они, тегами поверх стиля: стиль общий, и через него одну реплику не перекрасить,
не тронув остальные. Целое движение ползунка — один шаг отмены, а не сорок.

Видео открывается перетаскиванием: файл из проводника бросают прямо на кадр.
Так же открываются субтитры и проекты — программа сама разбирает, что ей
принесли, и пишет на кадре, что произойдёт, пока файл ещё висит над окном.

Открыв видео, программа читает его частоту кадров и запоминает её в проекте.
Если субтитры делались под другую частоту, она скажет об этом числом: не
«×1.001», а «к концу отстанут на 7 секунд» — и предложит пересчитать. Это тот
самый случай, когда в начале серии всё совпадает, а к финалу реплики
опаздывают: 23.976 против 24 дают миллисекунду на секунду, и она копится.
Отдельно то же лечится через «Тайминг → Коррекция под частоту кадров», с
предпросмотром «было → стало».
...
свой). Формат меняется в настройках, и уже расставленные имена переписываются
на новый лад тут же — одним шагом отмены на весь документ.
...
Под кадром — полоса с кнопками: новая реплика, дублировать, удалить, новая
дорожка. Кнопки подписаны и ростом с палец: это то, что делают по сотне раз за
смену, и лазить за этим в меню или целиться в мелкий значок незачем. Команды
те же, что в меню «Правка», и подсказка на кнопке показывает её сочетание
клавиш.
