...
Открывает и сохраняет ASS, SRT и WebVTT. ASS читается и пишется байт в байт:
...
Проверки качества: 10 правил, 4 профиля (общий, два по требованиям Netflix,
свободный). Скорость чтения, длина строк, длительность, зазоры, перекрытия,
несуществующие стили, непарные скобки, типографика.
...
счёта на видеокартах AMD и Intel, полного перевода интерфейса на другие языки.
...
1599 тестов, около пятнадцати секунд. Идут по файлам в отдельных процессах:
в одном процессе полторы тысячи тестов с Qt, libmpv и libass копят состояние,
и сессия начала обрываться падением интерпретатора в конце — при том, что все
тесты проходили.

Графические идут без экрана, для медиа генерируется настоящий MKV с заранее
известным содержимым — внешние файлы не нужны. Тесты, которым нужны libmpv
или libass, пропускаются, если библиотек нет.
