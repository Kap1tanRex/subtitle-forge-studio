...
OPAQUE_CALLS: frozenset[str] = frozenset({"compile", "match", "search", "sub", "fullmatch"})
...
def translatable(value: str) -> bool:
    ...
    Отсекаются три вида «строк», которые подписями не являются, но кириллицу
    содержат: многострочные куски (это обычно текст письма или regex с
    комментариями), literal с обратными слэшами (разметка ASS, шаблоны) и
    просто слишком длинные абзацы.
    """
    if len(value) > MAX_LENGTH:
        return False
    if chr(10) in value or chr(13) in value:
        return False
    # Обратный слэш — признак разметки ASS или шаблона, а не подписи.
