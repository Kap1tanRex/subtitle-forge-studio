...
class MpvPlayer:
    ...
    def __init__(self, *, video_output: str = "libmpv", **extra: object) -> None:
        ...
        # Плеер без картинки не должен занимать и звуковую карту. Такой
        # экземпляр служебный: им проверяют файл и гоняют тесты. Несколько
        # копий, одновременно открывших аудиоустройство, давали то зависание
        # прогона, то падение процесса — воспроизводилось при параллельных
        # тестах.
        if video_output == "null":
            options.setdefault("ao", "null")
