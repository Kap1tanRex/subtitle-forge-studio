...
HEADERS = ("№", "Начало", "Конец", "Важность", "Правило", "Замечание", "Текст")
...
def report_html(
    ...
    *,
    ...
    where = doc.source_path.name if doc.source_path else "без имени"
    parts.append(
        f'<p class="meta">{html.escape(where)} · '
        f'{plural(len(doc.events), "реплика", "реплики", "реплик")} · '
        f'найдено: {plural(len(issues), "замечание", "замечания", "замечаний")}</p>'
    )
...
def _profile_rows(profile: QcProfile) -> list[tuple[str, str]]:
    ...
    """
    named = (
        ("max_cps", "знаков в секунду, не более"),
        ("max_line_length", "знаков в строке, не более"),
        ("max_lines", "строк в реплике, не более"),
        ("min_duration_ms", "длительность, не менее (мс)"),
        ("max_duration_ms", "длительность, не более (мс)"),
        ("min_gap_frames", "зазор, не менее (кадров)"),
        ("shot_change_frames", "до склейки, не ближе (кадров)"),
        ("min_line_length", "знаков в строке, не менее"),
    )
    rows = []
